...
class Registrate extends StatefulWidget {
  ...
  @override
  State<Registrate> createState() =>
      _RegistrateState(); // Crea el estado asociado a esta pantalla.
}
...
class _RegistrateState extends State<Registrate> {
  ...
  final TextEditingController _confirmPasswordController =
      TextEditingController(); // Controlador para confirmar contraseña.
  ...
  bool _hasMinLength =
      false; // Indica si la contraseña tiene un mínimo de 6 caracteres.
  ...
  void initState() {
    ...
    _passwordController.addListener(
        _validatePassword); // Agrega un listener para validar la contraseña cuando cambia.
  }

  /// Método para validar los requisitos de la contraseña.
  void _validatePassword() {
    String password =
        _passwordController.text; // Obtiene el texto actual de la contraseña.
    setState(() {
      // Actualiza el estado según los requisitos de la contraseña.
      _hasUpperCase = password.contains(
          RegExp(r'[A-Z]')); // Verifica si hay al menos una letra mayúscula.
      _hasDigit = password
          .contains(RegExp(r'[0-9]')); // Verifica si hay al menos un dígito.
      _hasMinLength = password.length >=
          6; // Verifica si la longitud es de al menos 6 caracteres.
    });
  }
    ...
    if (_hasUpperCase && _hasDigit && _hasMinLength) {
      try {
        ...
        await userCredential.user
            ?.updateProfile(displayName: _nombreController.text);
        ...
        if (e.code == 'weak-password') {
          message =
              'La contraseña es demasiado débil.'; // Mensaje si la contraseña es débil.
        } else if (e.code == 'email-already-in-use') {
          message =
              'El correo electrónico ya está en uso.'; // Mensaje si el correo ya está en uso.
        }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.white, // Color de fondo transparente.
      appBar: AppBar(
        backgroundColor:
            Colors.transparent, // Barra de aplicación transparente.
          ...
          child: Column(
            children: [
              ...
              const Text(
                ...
                style: TextStyle(
                    fontSize: 24, color: Colors.white), // Estilo del subtítulo.
              ),
              ...
              _buildTextField(_correoController, Icons.email,
                  "Correo Electrónico"), // Campo para el correo.
              const SizedBox(height: 16), // Espacio entre campos.
              _buildTextField(_passwordController, Icons.lock, "Contraseña",
                  obscureText: true), // Campo para la contraseña.
              const SizedBox(height: 16), // Espacio entre campos.
              _buildTextField(_confirmPasswordController, Icons.lock,
                  "Confirmar Contraseña",
                  obscureText: true), // Campo para confirmar la contraseña.
              const SizedBox(height: 16), // Espacio entre campos.
              ...
              ElevatedButton(
                onPressed:
                    _registrar, // Método que se ejecuta al presionar el botón.
                style: ElevatedButton.styleFrom(
                  backgroundColor:
                      Colors.pink[400], // Color de fondo del botón.
                  padding: const EdgeInsets.symmetric(
                      vertical: 16, horizontal: 48), // Padding del botón.
                  shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(
                          90.0)), // Esquina redondeada del botón.
                ),
                child: const Text(
  ...
  Widget _buildTextField(
      ...
      {bool obscureText =
          false} // Indica si el texto debe ser oculto (para contraseñas).
      ) {
    return Container(
      padding: const EdgeInsets.symmetric(
          horizontal: 10, vertical: 5), // Padding del contenedor.
  ...
  Widget _buildPasswordRequirements() {
    return Column(
      children: [
        ...
        _buildRequirementText("Al menos una letra mayúscula",
            _hasUpperCase), // Requisito de letra mayúscula.
        _buildRequirementText(
            "Al menos un dígito", _hasDigit), // Requisito de dígito.
        _buildRequirementText("Mínimo 6 caracteres",
            _hasMinLength), // Requisito de longitud mínima.
      ],
    );
  }

  /// Método para construir el texto de los requisitos de la contraseña.
  Widget _buildRequirementText(String text, bool isMet) {
    return Row(
      children: [
        Icon(isMet ? Icons.check : Icons.close,
            color: isMet ? Colors.green : Colors.red), // Icono de cumplimiento.
        const SizedBox(width: 8), // Espacio entre el icono y el texto.
        Text(text,
            style:
                const TextStyle(color: Colors.black)), // Texto del requisito.
      ],
    );
  }
}
